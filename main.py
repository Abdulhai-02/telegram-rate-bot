...
# СПИСОК ID АДМИНИСТРАТОРОВ (Алик: старый и новый)
ADMIN_IDS = [5266659205]  
...
LANGS = {
    'ru': {
        ...
        'updated': "⏱ Обновлено:",
        'contact': "💰 Обмен любых сумм и валют — @Abdulkhaiii",
...
def save_auto(uid, changer="user"):
...
def main_keyboard(uid):
    ...
    if uid in ADMIN_IDS: m.row(LANGS[l]['btn_admin'])
    return m
...
def fetch_all_rates():
    ...
    def g_u():
        try: return float(requests.get("https://api.upbit.com/v1/ticker?markets=KRW-USDT", headers=headers, timeout=7).json()[0]["trade_price"])
        except: return None
    def g_b():
        try: return float(requests.get("https://api.bithumb.com/public/ticker/USDT_KRW", headers=headers, timeout=7).json()["data"]["closing_price"])
        except: return None
    def g_r():
        try: return 1_000_000 / requests.get("https://open.er-api.com/v6/latest/RUB", timeout=7).json()["rates"]["KRW"]
        except: return None
    def g_ab():
        try:
            d = requests.get("https://hub.abcex.io/api/v2/exchange/public/orderbook/depth?instrumentCode=USDTRUB", headers=headers, timeout=7).json()
...
def show_rate(m):
    ...
    if u is None and ab_buy is None:
...
# ============== АДМИНКА (ВЫСШИЙ УРОВЕНЬ) ==============
@bot.message_handler(func=lambda m: m.text in [LANGS['ru']['btn_admin'], LANGS['en']['btn_admin']] and m.from_user.id in ADMIN_IDS)
def admin_panel(m):
...
def admin_cb(c):
    ...
    if action == "stat":
        ...
        # Выводим список с кнопками подробнее
        for uid in list(ALL_USER_IDS)[-15:]: # Последние 15 для краткости
            d = USER_DATA.get(uid, {})
            name = d.get('first_name', 'User')
            kb = types.InlineKeyboardMarkup()
            kb.add(types.InlineKeyboardButton("🔍 Подробнее", callback_data=f"det_{uid}"))
            bot.send_message(c.message.chat.id, f"👤 {name} (<code>{uid}</code>)", reply_markup=kb, parse_mode="HTML")
        ...
        bot.edit_message_text("Интервал для ВСЕХ:", c.message.chat.id, c.message.message_id, reply_markup=kb)
        ...
        for uid in ALL_USER_IDS:
            ...
            save_auto(uid, changer="admin")
            count += 1
        bot.send_message(c.message.chat.id, f"✅ <b>Успешно!</b> Подписка {hours}ч активирована для {count} чел.")

    elif action == "auto_id":
        msg = bot.send_message(c.message.chat.id, "Введите ID юзера:")
...
# Детальная информация по кнопке
@bot.callback_query_handler(func=lambda c: c.data.startswith("det_"))
def admin_details(c):
    if c.from_user.id not in ADMIN_IDS: return
    ...
    if a:
        status = f"🟢 <b>ВКЛ</b> ({a['interval']//3600}ч)"
        last_auto = a['last'].strftime("%H:%M:%S (%d.%m)") if a['last'] else "—"
    else:
        status = "🔴 <b>ВЫКЛ</b>"
        last_auto = "—"

    joined = u['joined'].strftime("%d.%m.%Y %H:%M:%S")
    last_req = u['last'].strftime("%d.%m.%Y %H:%M:%S") if u['last'] else "Нет запросов"
    ...
    txt = (
        ...
        f"🆔 ID: <code>{uid}</code>\n"
        f"👤 Имя: {u['first_name']}\n"
        f"🔗 Ник: @{u['username'] if u['username'] else 'Нет'}\n"
        f"📅 Регистрация: <code>{joined}</code>\n"
        f"🖱 Последний запрос: <code>{last_req}</code>\n"
        f"📊 Всего запросов: {u['requests']}\n"
        f"━━━━━━━━━━━━━━\n"
        f"🔔 Автообновление: {status}\n"
        f"⏱ Последняя рассылка: {last_auto}"
    )
...
def adm_auto_step_id(m):
    try:
        ...
        msg = bot.send_message(m.chat.id, "Интервал (1, 5, 24):")
        bot.register_next_step_handler(msg, lambda s: adm_auto_final(s, tid))
    except: bot.send_message(m.chat.id, "Ошибка ID.")

def adm_auto_final(m, tid):
    try:
        h = int(m.text.strip())
        AUTO_USERS[tid] = {"interval": h * 3600, "last": now_msk()}
        save_auto(tid, changer="admin")
        bot.send_message(m.chat.id, f"✅ <b>Успешно!</b> Для {tid} включено {h}ч.")
...
def auto_callback(c):
    ...
    if val == 0:
        AUTO_USERS.pop(c.message.chat.id, None); save_auto(c.message.chat.id, "user")
        ...
        save_auto(c.message.chat.id, "user")
...
def disable_notif(m):
    AUTO_USERS.pop(m.chat.id, None); save_auto(m.chat.id, "user")
    bot.send_message(m.chat.id, "✅ <b>Успешно!</b> 🔕 Выключено.", parse_mode="HTML", reply_markup=main_keyboard(m.from_user.id))
...
def fb(m):
    ...
    bot.register_next_step_handler(msg, fb_s)

def fb_s(m):
    if m.text and len(m.text) > 2:
        log_action(m.from_user, "Отзыв", result=m.text)
        ...
        for cid, cfg in list(AUTO_USERS.items()):
            if (now - cfg["last"]).total_seconds() >= cfg["interval"]:
                try:
                    ...
                    AUTO_USERS[cid]["last"] = now; save_auto(cid, cfg.get("updated_by", "system"))
                except: pass
